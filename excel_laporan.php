...
// Cek autentikasi (Hanya Manager yang boleh akses cetak keseluruhan)
...
// Ambil Nama Manager yang sedang login untuk dicantumkan di subtitle header
...
// Set Header untuk memaksa browser mendownload sebagai file Excel (.xls)
header("Content-Type: application/vnd-ms-excel");
header("Content-Disposition: attachment; filename=Laporan_Seluruh_Riwayat_Pekerjaan_Daman_" . date('Ymd_His') . ".xls");
header("Pragma: no-cache");
header("Expires: 0");
?>
<table border="1" style="border-collapse: collapse; width: 100%;">
    <thead>
        <!-- BARIS UNTUK JUDUL LAPORAN (DITENGAH) -->
        <tr>
            <th colspan="13" style="text-align: center; font-size: 18px; height: 40px; vertical-align: middle;">
                LAPORAN SELURUH RIWAYAT PEKERJAAN DAMAN
            </th>
        </tr>
        <!-- BARIS UNTUK NAMA PENCETAK & TANGGAL UNDUH (DITENGAH) -->
        <tr>
            <th colspan="13" style="text-align: center; font-weight: normal; height: 30px; vertical-align: middle;">
                Dicetak Oleh: <b><?php echo strtoupper(htmlspecialchars($nama_manager)); ?></b> | Tanggal Unduh: <?php echo date('d-m-Y H:i:s'); ?>
            </th>
        </tr>
        <tr><th colspan="13" style="border: none; height: 15px;"></th></tr> <!-- Baris Kosong Pemisah -->

        <!-- HEADER DATA UTAMA (Warna Hijau Excel Premium) -->
        <tr style="background-color: #1D6F42; color: white; font-weight: bold; height: 25px; text-align: center;">
            ...
            <th style="white-space: nowrap;">Dikerjakan Oleh</th>
    ...
    <tbody>
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                ?>
                <tr>
                    <!-- mso-number-format:'\@' menjamin No Tiket tidak berubah format/hilang karakternya -->
                    ...
                    <td style="white-space: nowrap;"><b><?php echo htmlspecialchars($row['daman_name'] ? $row['daman_name'] : '-'); ?></b></td>
            ...
            echo "<tr><td colspan='13' style='text-align:center; height:30px; vertical-align:middle;'>Tidak ada data riwayat pekerjaan yang sesuai kriteria filter.</td></tr>";
        }
        ?>
    </tbody>
</table>